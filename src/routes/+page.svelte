<script lang="ts">
	...
	function generatePDF() {
		const resume = new GenerateResume(template1);
...
<div bind:this={resume} style="width: 500px; height: 834px;" class="">
	<div class="flex">
		<div class="flex-1">
			...
			<Typography variant="p" className="mt-2 text-xs">{$data.explain.join(' | ')}</Typography>
		</div>
		<div class="w-40">
			<Typography variant="p" className="text-xs">{$data.address}</Typography>
			<Typography variant="p" className="font-bold text-xs">{$data.phoneNumber}</Typography>
			<Typography variant="p" className="font-bold text-xs">{$data.email}</Typography>
		</div>
	</div>
	<div class="flex mt-3 gap-2">
		<div class="flex-1">
			<Typography variant="p" className="font-bold mb-3 text-sky-500">EXPERIENCE</Typography>
				...
				<div class="mb-3">
					...
					<ul class="list-disc">
						{#each experience.responsibilities as responsibility}
							<li>{responsibility.description} [{responsibility.languages.join(', ')}]</li>
						{/each}
					</ul>
				</div>
			{/each}
		...
		<div class="w-40">
			<Typography variant="p" className="font-bold mb-3 text-sky-500">SKILLS</Typography>
